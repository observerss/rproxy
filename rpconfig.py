...
class RPConfig:
    def __init__(self,path,domain="reverse-proxy.org"):
        self.path = path
        self.domain = domain
        self.cfgs = []
        self.cfgdict = {}
    ...
    def add_alias(self,target,alias,html,javascript,css,flash,extern):
        key = alias+"."+self.domain
        value = (target,alias,html,javascript,css,flash,extern)
    ...
    def _process(self,host,ctype,data):
        if self.cfgdict.has_key(host):
            target,alias,html,css,js,flash,extern = self.cfgdict[host]
    ...
    def read_config(self):
        ...
        for cfg in cfgs:
            ...
            if cfg:
                try:
                    target,alias,html,css,js,flash,extern = cfg.split()
                    args = self.format_check(target,alias,html,css,js,flash,extern)
...
# Each line is a rproxy entry, represents target,alias,html,css,js,flash,global
# the format is as follows:
# TARGET ALIAS Y/N Y/N Y/N Y/N Y/N
# e.g: 
# www.google.com gg Y Y Y Y Y
# obmem.com AUTO Y Y Y Y Y
'''
        cfgpath.write(docs)
        for cfg in self.cfgs:
            cfgpath.write("%s %s %s %s %s %s %s\n"%cfg)
    ...
    def format_check(self,target,alias,html,css,js,flash,extern):
        ...
        return (target,alias,html,css,js,flash,extern)

if __name__ == "__main__":
    rpconf = RPConfig("rproxy.cfg")
    rpconf.add_alias("playsc.com","playsc","Y","Y","Y","Y","Y")
